...
Imagine a client sending a "read_file" action. Instead of connecting directly to a specific file service, the client simply sends the action through this routing system, which then handles all complexities of routing, processing, and responding. This lets clients focus solely on their actions, while the system efficiently manages delivery to the correct handler.
...
1. **Broker**: Acts as the entry point for client messages, receiving actions like "read_file", generating a unique ID if needed, and routing the message to the appropriate message queue based on the specified action.
...
3. **Broker** routes the action to the **Action Channel** specifically designated for `"read_file"` actions.
4. **Broker** then establishes a persistent connection to a **Response UUID Channel**, where it will listen for a response tied to the action ID.
5. **Relay** connects to the **Action Channel** for `"read_file"` actions, where it receives the message and forwards it to the **Agent**.
...
```json
{
  "id": null,  // generated by the system if absent
  "type": "request",
  "action": "read_file",  // action the client wants to perform
...
### UUID Confirmation (from Broker to Client)
```json
{
  "id": "generated-uuid",
  "type": "uuid",
  "message": "UUID generated for tracking"
}
```

### Delivery Acknowledgement (from Broker to Client after routing to Relay)
```json
{
  "id": "generated-uuid",
  "type": "acknowledgement",
  "message": "Request delivered to agent"
}
```
